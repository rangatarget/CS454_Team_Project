...
def start_training(dataset, index_data):
    ...
    for i in range(len(index_data)):
        ...
        model_ = model.FixLocator(h_size=50, feature_representation_size=100, drop_out_rate=0.5, layer_num=5,
                                  code_cover_len=max_length)
        hit = train(epochs=1, trainLoader=train_dataset, testLoader=test_dataset, model=model_, learning_rate=0.0001)
        hit_all.append(hit)
    add_on = [0, 0, 0, 0, 0, 0]
    ...
    try:
        ...
        for e in range(epochs):
            for index, _data in enumerate(tqdm(trainLoader, leave=False)):
                out = model(_data)
                y_1 = torch.reshape(torch.tensor(_data.y), (-1,))
                out_1 = out[0].clone().detach().requires_grad_(True)
                loss = torch.autograd.Variable(criterion(out_1, y_1), requires_grad = True)
...
if __name__ == '__main__':
    dataset = []
    index_file = np.load(osp.join(os.getcwd(), 'custom_processed/index.npy', ), allow_pickle=True)
    for i in range(len(index_file)):
        for j in range(len(index_file[i])):
            data = torch.load(osp.join(os.getcwd(), 'custom_processed/data_{}.pt'.format(index_file[i][j])))
            dataset.append(data)
